...
Command:
`git grep -inwE "gate|gates|gated|brain|brawn|lane|lanes|cold|epic|grill|grilled|dag" -- skills/architect/`
...
- `git grep -c "Frozen check file path:" -- skills/architect/dispatch.md` → `2`
- `git grep -c "Checks integrity:" -- skills/architect/dispatch.md` → `2`
- `git grep -c "Per check:" -- skills/architect/dispatch.md` → `2`
- `git grep -c "## Stress-test delegation template" -- skills/architect/dispatch.md` → `1`
- `git grep -c "architect-stress-test-template:start" -- skills/architect/dispatch.md` → `1`
- `git grep -c "architect-grill-template" -- skills/architect/dispatch.md` → no output / zero matches (grep exits non-zero)
- `git grep -cE "^orchestrator = " -- skills/architect/dispatch.md` → `1` or more
- `git grep -cE "^builders = " -- skills/architect/dispatch.md` → `1` or more

## BS5 — new vocabulary present

Command:
`git grep -lE "docs/checks/" -- skills/architect/SKILL.md skills/architect/dispatch.md && git grep -lE "docs/jobs/" -- skills/architect/dispatch.md && git grep -liE "tracking issue" -- skills/architect/SKILL.md && git grep -liE "ready issues" -- skills/architect/loop.md && git grep -liE "hard stop" -- skills/architect/SKILL.md`
...
target file. Commands:
- `git grep -c "## Model alias table" -- skills/architect/dispatch.md` → `1`
- `git grep -c "## Issue conventions" -- skills/architect/dispatch.md` → `1`
- `git grep -c "## Monitor dispatch" -- skills/architect/dispatch.md` → `1`
- `git grep -c "## Respawn-with-answer template" -- skills/architect/dispatch.md` → `1`
- `git grep -c "## Factory block procedure" -- skills/architect/loop.md` → `1`

...
parse_fixture() {
  awk '
    /^- PROMPT: / {
      ...
      if (skill_line !~ /^[[:space:]]+SKILL: (architect|architect-research|codebase-design|to-spec|to-issues|frozen-checks|tdd|adversarial-review|final-review)$/) { printf "ERROR\tinvalid SKILL line after prompt near line %d: %s\n", NR - 1, skill_line > "/dev/stderr"; exit 2 }
      skill = skill_line

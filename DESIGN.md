...
**The design rationale for an autonomous software factory.** The brain model
(the session you open — Claude Fable 5 or Codex) runs intake, writes the spec,
decomposes it into a GitHub issue DAG, dispatches parallel cold builder lanes
into worktrees, answers blockers, sends cold judges against frozen gates, and
merges — with exactly one human step, the spec gate. This document is the
...
|---|---|---|
| **Orchestrator (brain)** | the session the human opened | intake, spec, decomposition, gate freeze, dispatch, blocker answers, merge decisions, digest |
| **Builder (brawn)** | cold worker agent, one per issue, own worktree | implementation and raw-evidence reporting only |
| **Judge** | cold brain-tier agent, read-only | frozen-gate verdicts and diff-vs-intent |
| **Monitor** | cheapest-tier background agent, one per wave | stall detection only — never kills, never decides |
| **Grill** | cold adversarial reviewer, pre-freeze | falsifying the decomposition before it's authorized |
| **Human** | you | the spec gate, stop rails, taste |

Why the brain does the design work and the brawn only builds:
...
Why the judge is a *separate cold context* at brain tier rather than the
...
frozen gates, rulings files, lane reports.
...
- **GitHub natively supports the whole graph.** Issue dependencies
  ...
  up to the lane cap" — no custom state files or body-text conventions.
- **One diary file was the highest-contention artifact in the repo.** Every
  lane's lifecycle wanted a row in it, which fought the disjoint-file-set
  rule that makes parallel lanes safe. Issues give each unit of work its own
  thread; comments are the append-only log.
- **The tracker is the human-visible dashboard.** PHASE-0 disagreements,
  blocker answers, verdicts, and the epic digest are readable from GitHub
...
### Intake and the spec gate

- **At most ~5 materiality-gated questions, in one batch (D3).** GitHub Spec
  ...
  established middle path between maximal grilling and zero questions
  ([Spec Kit](https://github.com/github/spec-kit);
  [factory research](docs/research/autonomous-software-factory.md)).
- **The spec gate is the one human step (D4).** The human reviews one
  `docs/spec/<project>.md`, edits or vetoes assumptions, and approves.
  Approval authorizes the entire issue DAG — after it, the human hears from
  the factory only through the epic digest or a stop rail. Concentrating
...
- **Backend canary before decomposition (v5.1 D1).** Every candidate brawn
  backend runs one trivial task proving it has a working shell executor; a
  DEGRADED backend is substituted *before* the DAG records tiers, with the
  evidence on the epic. Motivated by 6/6 Claude subagent spawns losing shell
  tools in one day (§7, D12) — backend choice is an intake-time risk, not a
  mid-wave surprise.

### Decomposition

- **Vertical slices as issues, dispatched by unblocked frontier (D5).** Each
  ...
  The parallel set is always the DAG's unblocked frontier, capped at five
  lanes plus one monitor.
- **Concurrently scheduled issues share nothing mutable.** Not files,
  ...
  conflict is therefore treated as a *decomposition* failure: kill the lane
...
- **Producer issues carry interface contract blocks.** An issue whose surface
  ...
  misinterpretation
  ([Anthropic multi-agent](https://www.anthropic.com/engineering/multi-agent-research-system)) —
  applied between lanes, and it avoided merge conflicts in the dogfood runs
  (§7).
- **Design-quality doctrine (D9).** Structural and behavioral changes are
  separate issues with a blocking edge (structural gates prove existing
  ...
  in the spec so lanes don't invent them mid-flight (TDD sourced from
...
- **Judged diffs target ≤ ~400 changed lines (P3).** Human review
  ...
  a spec that will exceed the target splits into more lanes or issues.

### Frozen gates and the grill

- **Gates freeze in git before results exist (R2 invariant, unchanged since
  ...
  criteria out of the agent's editable blast radius. Gates live in
  `docs/gates/<issue-slug>.md`, freeze at one commit, and **any builder edit
  under `docs/gates/` is an automatic FAIL regardless of results**. Visible
...
- **One cold grill pass attacks the whole decomposition (P2, widened by
  v5.1 D3).** Before the freeze, a read-only adversarial agent executes
  draft gate commands against the current tree, attacks acceptance criteria
  for non-falsifiability and repo-name grep collisions, sweeps references to
  files the DAG deletes or renames, and checks new artifact paths against
  ...
  blocking gate defects (§7).

### Building

- **Fresh cold builder per issue; respawn over resume (D7).** The Ralph
  ...
  When a lane blocks or wedges, the orchestrator answers durably on the
  issue and spawns a *new* cold lane with the answer in its spawn context —
...
- **PHASE 0: disagreement is mandatory.** Before building, every lane states
  its plan and every disagreement with the spec, citing real files — or what
  it checked before finding none. Silent compliance is a lane defect.
...
- **No silent fallbacks, no unrequested backcompat (P1).** Success-shaped
  defaults can fake a passing gate while the primary path is broken. Sources:
  ...
  limits are recorded and routed to a stop rail, never worked around
  silently.
- **Dispatch carries the full contract.** Objective, output format, tool
  guidance, boundaries, rulings, and the frozen-gate pointer travel in one
...
- **Nobody grades their own work.** The builder reports evidence; a cold
  brain-tier judge runs the frozen gates itself (builder claims are
  hearsay) and returns per-gate **PASS / FAIL / INVALID** — INVALID meaning
  "not measured the way the gate specifies", so unmeasured never equals
  passed ([Demystifying Evals](https://www.anthropic.com/engineering/demystifying-evals-for-ai-agents)).
- **The judge also reads the diff against intent.** Gate output alone is
...
- **The orchestrator may not turn a judge FAIL into a merge.** Verdicts are
  ...
  upon. Judgment context is pointer-only — frozen gate file, spec, lane
...
- **Post-freeze rulings live in an append-only file (v5.1 D4).**
  `docs/lanes/<issue-slug>-rulings.md`, orchestrator-owned, committed before
...
- **BLOCKED is a completion event.** The lane posts the exact blocker plus
...
- **The monitor detects; the brain rules (D6).** One cheapest-tier,
  ...
  the kill decision at brain tier keeps a cheap model from destroying an
  expensive healthy run. Quiet exit on an all-green wave is success, not
  waste.
- **Stop rails (D11).** `docs/STOP` before any wave; irreversible actions;
  two consecutive KILLs; a blocker colliding with a recorded assumption
  (a spec-gate decision surfacing late); scope growth beyond the approved
  spec; unsatisfiable preflight. All of them stop the factory and ask the
  human.

### Model routing

- **Brain, brawn, judge, and monitor are configurable roles, not brand
...
- **Default brawn is codex-first.** `codex/best` (gpt-5.5, xhigh) whenever
...
- **Everything lands on `factory/<run>`; main stays untouched until one
  closing PR.** The PR body closes the epic and lists every shipped issue;
  each closed issue gets a back-link comment naming the PR.
- **Docs debt batches into one dedicated lane at the PR boundary (P7).**
  Product docs (README, DESIGN) are the highest-contention files in the
  repo and evidence rows need post-judgment information, so build lanes and
  the orchestrator never edit them mid-run; pointers accumulate and one
  docs lane consumes them before the PR.
...
- **Scout-first, topic-designed lanes — no fixed taxonomy.** All five
  ...
  lines; the orchestrator designs 3–6 lanes from that map, drawing
  source-class tactics from `lanes.md`. Perspective discovery was STORM's
  largest measured lever (unique references 99.83 vs 54.36,
  [STORM](https://arxiv.org/abs/2402.14207)). Fact-finds and comparisons
  skip the scout.
- **Hard budgets per lane.** Researcher counts scale 1/2–4/4–6 by tier;
...
- **~2,500-token compact returns against a numbered source list (A1).**
  ...
  lanes at 2,000–3,600 tokens with citations; per-URL tag citations removed
...
|---|---|
| Reward hacking / gate tampering | Gates frozen in git pre-dispatch; `git diff` integrity check at judgment; tampering = automatic FAIL |
| Builder grades own work | Raw-evidence-only reports; cold judge runs gates itself; cross-family review for high-stakes |
| Goalpost moving | Verbatim frozen gate text; gates read-only after freeze; missing gate = spec defect for the *next* issue |
| Scope creep | Explicit boundaries and out-of-scope per issue; silent additions = lane failure; scope growth beyond spec = stop rail |
| Context rot | Orchestrator holds judgment only, never reads large diffs; fresh cold lane per issue; tracker + git carry state |
| Merge conflicts between lanes | Disjoint mutable-state sets, ≤5 lanes, worktrees; conflict = decomposition failure, re-spec |
| Placeholder implementations | End-to-end executable gate commands; "search before implementing; full implementations only" in the builder block |
...
| Gate-passing but unmergeable work | Judge reads diff vs intent, not gate output alone (METR) |
| Builder gaming visible gates | Frozen read-only gates; no iterate-against-judge loop (ImpossibleBench 33%→38%) |
| Stalled lanes | Detection-only monitor: growth + process + repeated-action checks; brain rules on evidence; no kill ceilings |
| Runaway factory | `docs/STOP`; two-consecutive-KILL rail; assumption-collision rail; epic digest as the human channel |
...
| Researcher context exhaustion | ≤5 subjects per lane; compact returns; bisect dead lanes |
...
- **Not same-session self-continuation.** Every unit of work gets a cold
  context; the orchestrator sleeps between events instead of accumulating
  lane output. Ralph-style same-session loops are the documented
...
- **Not just Goal Mode.** Codex's Goal Mode loops plan→act→test→review
  internally; this design adds what it lacks — frozen external gates, cold
  cross-model judgment, issue-DAG coordination, and repo-resident evidence.
- **Not human-free.** Autonomy is bounded by the spec gate at the front and
  stop rails throughout; kill/continue authority inside the run belongs to
  judges and rails, and taste stays with the human.
...
- **D11 — CLI spawns can ignore `isolation: worktree` (2026-07-02).** A CLI
  ...
  after spawn; never run two Claude-backend lanes concurrently without
...
- **D12 — intermittent, definition-asymmetric tool strip (2026-07-02).**
  ...
  (workspace-write, tree audited untouched) for shell-dependent gates, plus
  a cold headless `claude -p` session for gates the Codex sandbox cannot
...
- **Sandbox substitutions, recorded per gate:** `uv` AppData cache denial →
  ...
  paths and sequential gate execution.
- **Grill catch record:** 5 draft-gate defects (first use), 8 (second), 2
  blocking (third) — defects found on every use to date.
- **Dogfood runs.** v5 was built *by* the factory as a real issue DAG (epic
  #12, issues #13–#18): 1 judge FAIL, 3 respawns, all lanes cold-judged.
  The v5.1 hardening run (epic #20, issues #21–#25, on `factory/v5.1`)
  scored 0 judge FAILs and 0 respawns after the v5.1 decisions landed —
  canary preflight, freeze-push-dispatch, extended grill, rulings files.
